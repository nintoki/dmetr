...
export default class Search extends Component {
  ...
  render() {
    ...
    return (
		<div className="row">
			<div className="col-md-6" style={{maxWidth:'400px', marginBottom:'60px'}}>
				<h2 style={{marginBottom:'20px'}}>Patient Search</h2>
				<div style={{display:'inline-block', verticalAlign:'middle'}}>
					<input
						placeholder="Search by name or BT ID..."
						style={{height:'45px', width:'260px', padding:'5px 15px'}}
						name="searchTerm"
						id="searchTerm"
						value={ this.state.searchTerm }
						onChange={this.onInputChange}
						onKeyPress={!this.state.searchTerm
						?
						null : this.handleKeyPress}>
					</input>
				</div>
				<Link
					id="searchBtn"
